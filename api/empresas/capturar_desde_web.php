...
use App\Empresas\HtmlCompanyExtractor;

try {
    // Body JSON: { url, html, criterios: { sector, provincia?, ciudad?, etiquetas?[] } }
    $raw = file_get_contents('php://input');
    if (!$raw)
        throw new RuntimeException('BODY_VACIO');
    ...
    if ($url === '' || $html === '')
        throw new RuntimeException('FALTAN_PARAMETROS');
    ...
    if ($sector)
        $info['sector'] = $sector;
    if ($prov && empty($info['provincia']))
        $info['provincia'] = $prov;
    if ($ciu && empty($info['ciudad']))
        $info['ciudad'] = $ciu;
