<?php
// lib/empresas/Crawler.php
declare(strict_types=1);

require_once __DIR__ . '/HtmlCompanyExtractor.php';
...
final class Crawler
{
    ...
    private function extractCompanyLinks(string $html, string $baseUrl, string $domain, string $itemSelector, array $keywords): array
    {
    ...
    {
        $sel = $this->pdo->prepare("SELECT id FROM empresas WHERE (web = :web AND web <> '') OR (nombre = :nombre AND ciudad <=> :ciudad) LIMIT 1");
    ...
    {
        $hay = mb_strtolower(implode(' ', array_filter([$info['nombre']??'', $info['direccion']??'', $info['web']??''])));
        foreach ($kw as $k) {
    ...
    private function isNoise(string $url): bool
    {
        ...
        $ban  = ['facebook.com','twitter.com','x.com','instagram.com','linkedin.com','youtube.com','youtu.be','wa.me','t.me'];
