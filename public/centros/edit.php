...
$stE->execute([':id'=>$id]);
$centro = $stE->fetch();
if (!$centro) { http_response_code(404); exit('Centro no encontrado'); }
...
<head>
  <meta charset="utf-8">
  <title>Editar centro — Practicalia</title>
  ...
  <main class="max-w-6xl mx-auto p-4">
    ...
    <form method="post" class="bg-white p-6 rounded-2xl shadow space-y-4 mb-8">
      ...
      <div>
        ...
        <input name="nombre" value="<?= htmlspecialchars($centro['nombre']) ?>" required class="mt-1 w-full border rounded-xl p-2">
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label class="block text-sm font-medium">Teléfono</label>
          <input name="telefono" value="<?= htmlspecialchars($centro['telefono'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
        <div>
          <label class="block text-sm font-medium">Email</label>
          <input name="email" type="email" value="<?= htmlspecialchars($centro['email'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
      </div>

      <div>
        <label class="block text-sm font-medium">Web</label>
        <input name="web" value="<?= htmlspecialchars($centro['web'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="https://...">
      </div>

      <div>
        <label class="block text-sm font-medium">Dirección</label>
        <input name="direccion" value="<?= htmlspecialchars($centro['direccion'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          <label class="block text-sm font-medium">Ciudad</label>
          <input name="ciudad" value="<?= htmlspecialchars($centro['ciudad'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
        <div>
          <label class="block text-sm font-medium">Provincia</label>
          <input name="provincia" value="<?= htmlspecialchars($centro['provincia'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
        <div>
