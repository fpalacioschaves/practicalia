  ...
  try {
    ...
    if ($profes) {
      ...
      foreach ($profes as $pid) {
        $pid = (int)$pid;
        if ($pid>0) $ins->execute([':c'=>$id, ':p'=>$pid]);
      }
    }
...
$stC->execute([':id'=>$id]);
$curso = $stC->fetch();
if (!$curso) { http_response_code(404); exit('Curso no encontrado'); }
...
<head>
  <meta charset="utf-8">
  <title>Editar curso — Practicalia</title>
  ...
  <main class="max-w-xl mx-auto p-4">
    ...
    <form method="post" class="bg-white p-6 rounded-2xl shadow space-y-4">
      ...
      <div>
        ...
        <input name="nombre" value="<?= htmlspecialchars($curso['nombre']) ?>" required class="mt-1 w-full border rounded-xl p-2">
      </div>

      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="block text-sm font-medium">Código</label>
          <input name="codigo" value="<?= htmlspecialchars($curso['codigo'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
        <div>
          <label class="block text-sm font-medium">Año</label>
          <input name="anyo" type="number" value="<?= htmlspecialchars((string)($curso['anyo'] ?? '')) ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
      </div>
      ...
      <div>
        ...
        <div class="mt-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
          <?php foreach ($profesores as $p): $pid=(int)$p['id']; ?>
