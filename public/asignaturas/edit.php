...
if (!$asig) { http_response_code(404); exit('Asignatura no encontrada'); }
...
if (!$isAdmin) {
  if (empty($seleccionados)) {
    http_response_code(403); exit('No tienes acceso a esta asignatura.');
  }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['accion'] ?? '') === 'actualizar_asignatura')) {
  try {
    ...
    if (!$data['cursos']) throw new RuntimeException('Selecciona al menos un grado.');
    ...
    foreach ($data['cursos'] as $cid) {
      if ($data['codigo'] !== '') {
        ...
        if ($stDupCode->fetch()) throw new RuntimeException('Ya existe una asignatura con ese código en el grado seleccionado.');
      }
      ...
      if ($stDupName->fetch()) throw new RuntimeException('Ya existe una asignatura con ese nombre en el grado seleccionado.');
    }
...
<head>
  <meta charset="utf-8">
  <title>Editar asignatura — Practicalia</title>
    ...
    <form method="post" class="bg-white p-6 rounded-2xl shadow space-y-4">
      ...
      <div class="grid grid-cols-1 md-grid-cols-2 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input name="codigo" value="<?= h($data['codigo']) ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="p.ej. SIGDAW2">
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          ...
          <input name="ects" inputmode="decimal" value="<?= h($data['ects']) ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="6 o 6.0">
        </div>
        <div>
          <label class="block text-sm font-medium">Horas</label>
          <input name="horas" type="number" min="0" max="2000" value="<?= h($data['horas']) ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
        <div>
      ...
      <div>
        ...
        <textarea name="descripcion" rows="4" class="mt-1 w-full border rounded-xl p-2"><?= h($data['descripcion']) ?></textarea>
      </div>
