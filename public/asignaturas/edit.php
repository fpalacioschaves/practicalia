...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['accion'] ?? '') === 'actualizar_asignatura')) {
  try {
    ...
    $data['nivel'] = trim($_POST['nivel'] ?? $data['nivel']);
    ...
    $stU = $pdo->prepare("
      UPDATE asignaturas SET
        curso_id=:curso_id, nivel=:nivel, nombre=:nombre, codigo=:codigo, ects=:ects,
...
<form method="post" class="bg-white p-6 rounded-2xl shadow space-y-4">
  ...
  <div>
    ...
    <textarea name="descripcion" rows="4" class="form-control"><?= h($data['descripcion']) ?></textarea>
  </div>
