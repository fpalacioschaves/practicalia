...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if ($password === '' || $password2 === '')
      throw new RuntimeException('Debes indicar y confirmar la contraseña.');
    if ($password !== $password2)
    ...
    foreach ($rolesSel as $rid) {
      foreach ($roles as $r) {
        if ((int) $r['id'] === $rid && $r['codigo'] === 'profesor') {
          $esProfesor = true;
          break 2;
        }
      }
    ...
    if ($esProfesor) {
      if ($cursoId <= 0)
        throw new RuntimeException('Para el rol profesor debes seleccionar un curso/familia.');
...
<form method="post" class="bg-white p-6 rounded-2xl shadow space-y-4">
  ...
  <div class="grid grid-cols-2 gap-3">
    <div>
      ...
      <input name="nombre" required class="form-control" value="<?= htmlspecialchars($_POST['nombre'] ?? '') ?>">
    </div>
    <div>
      <label class="block text-sm font-medium">Apellidos</label>
      <input name="apellidos" class="form-control" value="<?= htmlspecialchars($_POST['apellidos'] ?? '') ?>">
    </div>
  </div>
      ...
      <?php foreach ($roles as $r): ?>
        <label class="inline-flex items-center gap-2">
          ...
          <span><?= htmlspecialchars($r['nombre']) ?> <span
              class="text-xs text-gray-500">(<?= htmlspecialchars($r['codigo']) ?>)</span></span>
        </label>
      <?php endforeach; ?>
    </div>
  </div>
