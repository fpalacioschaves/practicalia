...
$pageTitle = 'Prospectos de empresas';
...
<form class="bg-white p-4 rounded-2xl shadow grid grid-cols-1 md:grid-cols-5 gap-3">
  <input class="border rounded-xl p-2 md:col-span-2" name="q" placeholder="Buscar por nombre, web, email, etiquetas..."
    value="<?= h($q) ?>">
  ...
  <select class="border rounded-xl p-2" name="curso_id">
    ...
    <?php foreach ($cursos as $c): ?>
      <option value="<?= (int) $c['id'] ?>" <?= $curso === (int) $c['id'] ? 'selected' : '' ?>><?= h($c['nombre']) ?>
      </option>
    <?php endforeach; ?>
  </select>
...
<div class="bg-white rounded-2xl shadow overflow-x-auto">
  <table class="min-w-full text-sm">
    ...
    <tbody>
      <?php if (!$rows): ?>
        <tr>
          <td colspan="7" class="p-4 text-gray-500">Sin resultados.</td>
        </tr>
      <?php else:
        foreach ($rows as $r): ?>
          <tr class="border-t">
            <td class="p-3">
              ...
              <div class="text-xs text-gray-600">
                <?php if ($r['web']): ?><a class="underline" target="_blank"
                    href="<?= h($r['web']) ?>"><?= h($r['web']) ?></a><?php endif; ?>
                <?= $r['web'] && ($r['email'] || $r['telefono']) ? ' · ' : '' ?>
                <?php if ($r['email']): ?><a class="underline"
                    href="mailto:<?= h($r['email']) ?>"><?= h($r['email']) ?></a><?php endif; ?>
                <?= $r['email'] && $r['telefono'] ? ' · ' : '' ?>
            ...
            <td class="p-3">
              <?php if ($r['responsable_nombre']): ?>
                <div>
                  <?= h($r['responsable_nombre']) ?>      <?= $r['responsable_cargo'] ? ' — ' . h($r['responsable_cargo']) : '' ?>
                </div>
                <div class="text-xs text-gray-600">
                  <?php if ($r['responsable_email']): ?><a class="underline"
                      href="mailto:<?= h($r['responsable_email']) ?>"><?= h($r['responsable_email']) ?></a><?php endif; ?>
            ...
            <td class="p-3">
              <div class="flex gap-2">
                ...
                <form method="post" action="./promote.php"
                  onsubmit="return confirm('¿Convertir este prospecto en empresa?');">
