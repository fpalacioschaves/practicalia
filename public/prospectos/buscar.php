...
if ($hasBing) $providers['bing'] = 'Bing Web Search';
if ($hasGCS)  $providers['gcs']  = 'Google Custom Search';
if ($hasSerp) $providers['serp'] = 'SerpAPI (Google)';
...
function nominatim_geocode_bbox(string $city, string $province): ?array {
  ...
  if (!$json) return null;
  $r = $json[0] ?? null;
  if (!$r || empty($r['boundingbox'])) return null;
...
function nominatim_search(string $query, string $city, string $province, int $limit = 20): array {
  ...
  foreach ($json as $it) {
    $name = '';
    if (!empty($it['namedetails']['name']))       $name = (string)$it['namedetails']['name'];
    elseif (!empty($it['display_name']))          $name = preg_replace('/,.*/', '', (string)$it['display_name']);
    else                                          $name = 'Empresa';

    $addr = (string)($it['display_name'] ?? '');
    $xt   = (array)($it['extratags'] ?? []);
    $urlSite = '';
    foreach (['website','contact:website','url','contact:url'] as $k) { if (!empty($xt[$k])) { $urlSite = (string)$xt[$k]; break; } }
    $phone = '';
    foreach (['phone','contact:phone'] as $k) { if (!empty($xt[$k])) { $phone = (string)$xt[$k]; break; } }

    // Enlace OSM correcto: /{node|way|relation}/{id}
    $osmLink = '';
    if (!empty($it['osm_type']) && !empty($it['osm_id'])) {
      $t = strtolower((string)$it['osm_type']);
      if (!in_array($t, ['node','way','relation'], true)) $t = 'node';
...
QL;
  ...
  foreach ($elements as $el) {
    ...
    foreach (['addr:street','addr:housenumber','addr:postcode','addr:city','addr:state'] as $k) {
      if (!empty($tags[$k])) $addrParts[] = $tags[$k];
    }
    ...
    foreach (['website','contact:website','url','contact:url'] as $k) {
      if (!empty($tags[$k])) { $site = $tags[$k]; break; }
    }
    $phone = '';
    foreach (['phone','contact:phone'] as $k) {
      if (!empty($tags[$k])) { $phone = $tags[$k]; break; }
    }

    // Enlace OSM correcto: /{node|way|relation}/{id}
    $osmType = strtolower((string)($el['type'] ?? 'node'));
    if (!in_array($osmType, ['node','way','relation'], true)) $osmType = 'node';
    $osmId   = (string)($el['id'] ?? '');
...
function search_bing(string $q, int $count = 10): array {
  ...
  if ($key === '') throw new RuntimeException('Falta BING_API_KEY');
...
function search_gcs(string $q, int $count = 10): array {
  ...
  if ($api === '' || $cx === '') throw new RuntimeException('Faltan GOOGLE_API_KEY/GOOGLE_CSE_ID');
...
function search_serp(string $q, int $count = 10): array {
  ...
  if ($key === '') throw new RuntimeException('Falta SERPAPI_KEY');
...
function or_group(array $terms): string {
  ...
  if (!$terms) return '';
  if (count($terms) === 1) return '"'.addslashes($terms[0]).'"';
...
function build_search_query(array $opts): string {
  ...
  $basePieces = [];
  if ($seedGroup !== '' && $sectorGroup !== '')       $basePieces[] = '('.$seedGroup.' OR '.$sectorGroup.')';
  elseif ($seedGroup !== '')                          $basePieces[] = $seedGroup;
  elseif ($sectorGroup !== '')                        $basePieces[] = $sectorGroup;
  if ($locPart !== '')                                $basePieces[] = $locPart;
  if ($mustPart !== '')                               $basePieces[] = $mustPart;
  ...
  if ($antiruido) {
    $noise = [
      '-empleo','-oferta','-ofertas','-trabajo','-prácticas','-practicas',
      '-infojobs','-indeed','-linkedin','-glassdoor',
      '-github','-gitlab','-stackoverflow','-stack overflow',
      '-pdf','-blog','-foro','-noticia','-wikipedia'
    ];
    $q .= ' ' . implode(' ', $noise);
  }
...
function guess_name_from_result(string $title, string $url): string {
  $title = trim($title);
  if ($title !== '') { $title = preg_replace('/\s+[\|\-–]\s+.*/u', '', $title); }
  if ($title !== '') return mb_substr($title, 0, 190);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($accion === 'generar_semillas') {
    try {
      ...
      if (!$cursoIds) throw new RuntimeException('No hay cursos disponibles para generar semillas.');
      ...
      if ($semillas === '') $diagMsg .= ' · Aviso: no hay asignaturas activas asociadas a esos cursos.';
    } catch (Throwable $e) {
      $error = $e->getMessage();
      $semillas = '';
    }
  ...
  if ($accion === 'buscar') {
    try {
      ...
      if ($prov === 'overpass') {
        $parts = [];
        if ($seeds !== '')  $parts[] = $seeds;
        if ($sector !== '') $parts[] = $sector;
        if ($must   !== '') { foreach (array_values(array_filter(preg_split('/\s+/', $must))) as $w) $parts[] = $w; }
        ...
        $fn = match ($prov) {
          ...
          'gcs'  => 'search_gcs',
          'serp' => 'search_serp',
          default => throw new RuntimeException('Proveedor no disponible'),
        };
  ...
  if ($accion === 'importar') {
    try {
      ...
      $n = 0;
      foreach ($sel as $idx) {
        if (!isset($rows[$idx]) || !is_array($rows[$idx])) continue;
        $r = $rows[$idx];
...
<head>
  <meta charset="utf-8">
  <title>Buscar empresas — Practicalia</title>
...
<body class="bg-gray-50 min-h-screen">
  ...
  <main class="max-w-6xl mx-auto p-4 space-y-6">
    ...
    <section class="bg-white p-6 rounded-2xl shadow space-y-4">
      ...
      <form method="post" class="space-y-3">
        ...
        <div>
          ...
          <textarea name="semillas" rows="3" class="w-full border rounded-xl p-2" placeholder="Bases de datos, 0484, desarrollo, web..."><?= h($postedSemillas) ?></textarea>
          <p class="text-xs text-gray-500 mt-1">Se usarán en la búsqueda si marcas “Construir consulta desde mis grados/asignaturas”.</p>
        </div>
      </form>
    </section>
      ...
      <form method="post" class="space-y-4" id="form-buscar">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
          <div class="md:col-span-3">
            ...
            <input name="q" class="mt-1 w-full border rounded-xl p-2" placeholder='p.ej. "empresa desarrollo web prácticas FP"'
                   value="<?= h($_POST['q'] ?? '') ?>">
          ...
          <div>
            ...
            <select name="provider" id="provider" class="mt-1 w-full border rounded-xl p-2">
              <?php foreach ($providers as $k=>$v): ?>
                <option value="<?= h($k) ?>" <?= ($selectedProv===$k)?'selected':'' ?>><?= h($v) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div>
            ...
            <select name="mode" id="mode" class="mt-1 w-full border rounded-xl p-2">
              ...
              ?>
                <option value="<?= h($k) ?>" <?= $postedMode===$k?'selected':'' ?>><?= h($v) ?></option>
              <?php endforeach; ?>
            </select>
            <p class="text-xs text-gray-500 mt-1">El modo aplica solo a proveedores web (Bing/GCS/Serp), no a Overpass/Nominatim.</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
            ...
            <input name="ciudad" class="mt-1 w-full border rounded-xl p-2" placeholder="Málaga, Sevilla, Granada..." value="<?= h($_POST['ciudad'] ?? '') ?>">
          </div>
          <div>
            <label class="block text-sm font-medium">Provincia</label>
            <input name="provincia" class="mt-1 w-full border rounded-xl p-2" placeholder="Málaga, Sevilla..." value="<?= h($_POST['provincia'] ?? '') ?>">
          </div>
          <div>
            <label class="block text-sm font-medium">Sector / palabras</label>
            <input name="sector" class="mt-1 w-full border rounded-xl p-2" placeholder="software, frontend, cloud..." value="<?= h($_POST['sector'] ?? '') ?>">
          </div>
          <div>
            <label class="block text-sm font-medium">Debe contener</label>
            <input name="must" class="mt-1 w-full border rounded-xl p-2" placeholder="prácticas, dual, FP..." value="<?= h($_POST['must'] ?? '') ?>">
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input type="number" min="1" max="20" name="num" class="mt-1 w-full border rounded-xl p-2" value="<?= h($_POST['num'] ?? '10') ?>">
          </div>
        </div>
        ...
        <div class="overflow-x-auto border rounded-2xl">
          <table class="min-w-full text-sm">
            <thead class="bg-gray-50">
              <tr>
                <th class="p-3">
                  <input type="checkbox" onclick="document.querySelectorAll('[data-row]').forEach(cb=>cb.checked=this.checked)">
                </th>
            ...
            <tbody>
              ...
              <tr class="border-t">
                ...
                <td class="p-3 align-top font-medium"><?= h($name) ?></td>
                <td class="p-3 align-top">
                  <?php if (!empty($r['url'])): ?>
                    <a class="underline break-all" target="_blank" href="<?= h($r['url']) ?>"><?= h($r['url']) ?></a>
                    <?php if (!empty($r['_maps'])): ?>
                      <div class="text-xs"><a class="underline text-gray-600" target="_blank" href="<?= h($r['_maps']) ?>">Ver en OpenStreetMap</a></div>
                    <?php endif; ?>
                ...
                <td class="p-3 align-top"><?= h($r['snippet'] ?? '') ?></td>
              </tr>
    ...
    <section class="bg-white p-6 rounded-2xl shadow space-y-4">
      ...
      <p class="text-sm text-gray-600">Formato por línea (opcional): <code>Nombre | web | email | teléfono | ciudad | provincia</code></p>
      <form method="post" class="space-y-3">
        <input type="hidden" name="csrf" value="<?= csrf_html() ?>">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input name="semillas" value="<?= h($postedSemillas) ?>" class="w-full border rounded-xl p-2" placeholder="DAW, DAM, web, backend">
          </div>
        </div>

        <div>
          <label class="block text-sm font-medium mb-1">Listado</label>
          <textarea name="prospectos_raw" rows="6" class="w-full border rounded-2xl p-3" placeholder="MiEmpresa SL | https://miep.com | rrhh@miep.com | 600000000 | Málaga | Málaga"></textarea>
        </div>
