...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['accion'] ?? '')==='guardar') {
  try {
    ...
    if ($responsable_email!=='' && !filter_var($responsable_email, FILTER_VALIDATE_EMAIL)) throw new RuntimeException('Email del responsable no válido.');
    if (!in_array($estado, ['nuevo','pendiente','contactada','interesada','descartada'], true)) $estado='nuevo';
    if (!in_array($origen, ['manual','busqueda','import'], true)) $origen='manual';
    ...
    if ($id) {
      ...
      ");
      $st->execute([
        ':id'=>$id, ':nombre'=>$nombre, ':sector'=>$sector, ':cnae'=>$cnae,
        ':web'=>($web!==''?$web:null), ':email'=>($email!==''?$email:null), ':telefono'=>($telefono!==''?$telefono:null),
        ':rnom'=>($responsable_nombre!==''?$responsable_nombre:null), ':rcargo'=>($responsable_cargo!==''?$responsable_cargo:null),
        ':remail'=>($responsable_email!==''?$responsable_email:null), ':rtel'=>($responsable_telefono!==''?$responsable_telefono:null),
        ':ciudad'=>($ciudad!==''?$ciudad:null), ':provincia'=>($provincia!==''?$provincia:null), ':notas'=>($notas!==''?$notas:null),
        ':estado'=>$estado, ':origen'=>$origen, ':fuente_url'=>($fuente!==''?$fuente:null), ':tags'=>($tags!==''?$tags:null),
        ':curso'=>($cursoId>0?$cursoId:null), ':prof'=>($asignado>0?$asignado:null),
      ]);
    } else {
      // insert
      $st = $pdo->prepare("
      ...
      ");
      $st->execute([
        ':nombre'=>$nombre, ':sector'=>$sector, ':cnae'=>$cnae,
        ':web'=>($web!==''?$web:null), ':email'=>($email!==''?$email:null), ':telefono'=>($telefono!==''?$telefono:null),
        ':rnom'=>($responsable_nombre!==''?$responsable_nombre:null), ':rcargo'=>($responsable_cargo!==''?$responsable_cargo:null),
        ':remail'=>($responsable_email!==''?$responsable_email:null), ':rtel'=>($responsable_telefono!==''?$responsable_telefono:null),
        ':ciudad'=>($ciudad!==''?$ciudad:null), ':provincia'=>($provincia!==''?$provincia:null), ':notas'=>($notas!==''?$notas:null),
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['accion'] ?? '')==='guardar_contacto') {
  try {
    ...
    if ($prospectoId <= 0) throw new RuntimeException('Prospecto inválido.');
    if ($id && $prospectoId !== $id) throw new RuntimeException('IDs no coinciden.');
    ...
    ");
    $st->execute([
      ':p'=>$prospectoId, ':u'=>$profId, ':f'=>$fecha, ':c'=>$canal, ':a'=>$asunto,
      ':r'=>($resumen!==''?$resumen:null), ':res'=>($resultado!==''?$resultado:null),
      ':pa'=>($prox!==''?$prox:null), ':conf'=>$confid,
    ]);
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['accion'] ?? '')==='eliminar') {
  try {
    ...
    if ($prospectoId <= 0) throw new RuntimeException('ID inválido.');
...
<head>
  <meta charset="utf-8">
  <title><?= $id ? 'Editar prospecto' : 'Nuevo prospecto' ?> — Practicalia</title>
    ...
    <form method="post" class="bg-white p-6 rounded-2xl shadow space-y-4 mt-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <input name="nombre" required value="<?= h($prospecto['nombre'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
        <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          ...
          <input name="cnae" value="<?= h($prospecto['cnae'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="6201, 6311...">
        </div>
        <div>
          <label class="block text-sm font-medium">Web</label>
          <input name="web" value="<?= h($prospecto['web'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="https://...">
        </div>
        <div>
          <label class="block text-sm font-medium">Email</label>
          <input type="email" name="email" value="<?= h($prospecto['email'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
        </div>
      </div>
      ...
      <div class="pt-4 border-t">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input name="responsable_nombre" value="<?= h($prospecto['responsable_nombre'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
          </div>
          <div>
            <label class="block text-sm font-medium">Cargo</label>
            <input name="responsable_cargo" value="<?= h($prospecto['responsable_cargo'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          <div>
            <label class="block text-sm font-medium">Email</label>
            <input type="email" name="responsable_email" value="<?= h($prospecto['responsable_email'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
          </div>
          <div>
            <label class="block text-sm font-medium">Teléfono</label>
            <input name="responsable_telefono" value="<?= h($prospecto['responsable_telefono'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2">
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <input name="prospecto_etiquetas" value="<?= h($prospecto['prospecto_etiquetas'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="DAW, DAM, front, backend...">
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div class="md:col-span-2">
          ...
          <input name="fuente_url" value="<?= h($prospecto['fuente_url'] ?? '') ?>" class="mt-1 w-full border rounded-xl p-2" placeholder="https://resultados-busqueda/...">
        </div>
      </div>

      <div>
        <label class="block text-sm font-medium">Notas</label>
        <textarea name="notas" rows="4" class="mt-1 w-full border rounded-xl p-2"><?= h($prospecto['notas'] ?? '') ?></textarea>
      </div>
        ...
        <div>
          ...
          <select name="asignado_profesor_id" class="mt-1 w-full border rounded-xl p-2">
            <?php foreach ($profs as $p): ?>
              <option value="<?= (int)$p['id'] ?>" <?= (int)$asig===(int)$p['id']?'selected':'' ?>><?= h($p['nombre']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
      <?php endif; ?>
      ...
      <section id="contactos" class="bg-white p-6 rounded-2xl shadow mt-8">
        ...
        <div class="overflow-x-auto border rounded-2xl">
          <table class="min-w-full text-sm">
            ...
            <tbody>
              <?php if (!$contactos): ?>
                <tr><td colspan="6" class="p-3 text-gray-500">Sin contactos todavía.</td></tr>
